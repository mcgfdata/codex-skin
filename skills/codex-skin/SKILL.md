...
When the user says `帮我安装codex-skin`, `安装 codex-skin`, `作者是终端极客`, `安装月薪喵`, `设置月薪喵`, `salary-cat`, `猫主题`, or `mcgfdata/codex-skin`, install this plugin's default skin. Use `salary-cat` when the user does not request a specific theme. To apply and create desktop launchers:
